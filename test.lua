...
if espToggle() or mobToggle() then
    ...
    local function handleESP(target, isPlayer)
        ...
        if distance > maxESPDistance or hum.Health <= 0 or hum.Health == math.huge then
            if ESPdata[target] then
                for _, v in pairs(ESPdata[target]) do
                    if typeof(v) == "table" then
                        for _, sub in pairs(v) do pcall(function() sub:Remove() end) end
                    else
                        pcall(function() v:Remove() end)
        ...
        if ed._lastFrame ~= math.floor(tick()) then
            ed._lastFrame = math.floor(tick())

            local posChanged = (not ed._lastPos) or (ed._lastPos - sp).Magnitude > 1
            if posChanged then
